...
`ikcp_input` 之后，上层调用 `ikcp_update` 时候会发送 ACK 包，调用 `ikcp_recv` 会给上层返回有效数据。`ikcp_update` 和 `ikcp_recv` 互相独立，没有调用顺序要求，视上层的调用时机而定。我们先来看 `ikcp_update` 里面有关 ACK 发送的buff：
