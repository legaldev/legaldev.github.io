...
``` cpp
#define ADD_COMMA(arg0, arg1) arg0, arg1
...
在重复展开的过程中，禁止递归展开相同的宏。可以把宏展开理解为树形的结构，根节点就是一开始要展开的宏，每个宏展开之后的内容作为该宏的子节点连接到树上，那么禁止递归就是在展开子节点的宏时候，如果该宏跟任意祖先节点的宏相同，则禁止展开。来看一些例子：
...
`IDENTITY_IMPL(CONCAT(CON, CAT(a, b)))`：`IDENTITY_IMPL` 可以理解为对参数 `arg0` 求值，这里的参数 `arg0` 求值得到了 `CONCAT(a, b)`，并由于递归被标记为了禁止展开，之后 `IDENTITY_IMPL` 展开完成，进行第二次扫描的时候，发现是禁止展开的 `CONCAT(a, b)`，于是停止展开。在这里 `CONCAT(a, b)` 是由参数 `arg0` 展开而得到的，但在后续展开的时候，也会保持禁止展开的标记，可以理解为父节点是参数 `arg0`，一直保持着禁止递归的标记。

`IDENTITY(CONCAT(CON, CAT(a, b)))`：这个例子主要是为了加强对父子节点的理解，参数自己展开的时候，是自身作为父节点，展开的内容作为子节点去判断递归，展开后的参数传到宏定义之后，禁止展开的标记时保留的（如果传到宏定义之后没有改变参数展开后的宏）。例如这里，在第一次完全展开之后得到 `IDENTITY_IMPL(CONCAT(a, b))`，但 `CONCAT(a, b)` 已经标记为禁止展开， `IDENTITY_IMPL` 是对参数求值的，但参数已经禁止展开，所以参数就原封不动的传到定义里，最后我们还是得到 `CONCAT(a, b)`。

我这里只是列出了一些比较重要的，或者我觉得不太好理解的规则，详细的具体的宏展开规则，还是建议花点时间直接去看标准文档。

## 通过 Clang 观察展开过程

我们可以给 Clang 源码加上一些打印信息来观察宏展开的过程，我无意深入解释 Clang 的源码，在这里给一份修改过的文件，有兴趣的可以自己编译 Clang 来研究。这里我是用的 llvm 版本 11.1.0，**[传送门](https://github.com/llvm/llvm-project/archive/refs/tags/llvmorg-11.1.0.tar.gz)**，修改的文件 **[传送门]({{PAGE_ASSET_PATH}}/clang_modified.zip)**。下面简单通过例子来验证我们之前介绍的宏展开规则：
...
HandleIdentifier: 
MacroInfo 0x561a941c5760
    #define <macro>(arg0, arg1) arg0 ## arg1
Macro is ok to expand

EnterMacro

Enter ExpandFunctionArguments: 
MacroInfo 0x561a941c5760 used
    #define <macro>(arg0, arg1) arg0 ## arg1
Token: 0
identifier: arg0
Args: [identifier: C]
...
HandleIdentifier: 
MacroInfo 0x561a941c5760 disabled used
    #define <macro>(arg0, arg1) arg0 ## arg1
...
* 第 1 行 `HandleIdentifier` 遇到宏的时候会打印，接着打印宏的定义信息（2-3 行），宏没有禁用，所以可以按照定义来展开 `Macro is ok to expand`，之后进入宏 `EnterMacro`。
...
* 第 0 个 `Token` 是形参 `arg0`, 对应的实参是 `C`，判断不需要展开，于是直接复制到结果上。

* 第 1 个 `Token` 是 `hashhash`，也就是 `##` 操作符，继续复制到结果上。

* 第 2 个 `Token` 是形参 `arg0`，对应的实参是 `ONCAT(a, b)`，预处理器也会把实参处理成一个个的 `Token`，所以可以看到打印的结果用中括号包住了实参的每个 `Token`（第 18 行），由于 `##` 的原因这个实参依然不需要展开，所以还是直接复制到结果上。

* 最后 `ExpandFunctionArguments` 就是一次扫描展开得到的结果（第 19 行），把结果的 `Token` 都翻译过来就是 `C ## ONCAT(a, b)`，之后预处理器就执行 `##` 操作符来生成新的内容。

* `##` 执行之后得到 `CONCAT(a, b)`，遇到宏 `CONCAT`，预处理还是先进入 `HandleIdentifier`，打印宏的信息，发现该宏 `disable used`，是已经展开过的，禁止再展开了，于是 `Macro is not ok to expand`，预处理不再展开，最终得到的结果就是 `CONCAT(a, b)`。
...
<details>
<summary> <font color=red> Clang 得到 （点击展开）：</font> </summary>
{% highlight text linenos %}
HandleIdentifier: 
MacroInfo 0x55c5d17e7890
    #define <macro>(arg0) arg0
Macro is ok to expand

HandleIdentifier: 
MacroInfo 0x55c5d17e7760
    #define <macro>(arg0, arg1) arg0 ## arg1

EnterMacro

Enter ExpandFunctionArguments: 
MacroInfo 0x55c5d17e7890 used
    #define <macro>(arg0) arg0
Token: 0
identifier: arg0
...
HandleIdentifier: 
MacroInfo 0x55c5d17e7760 disabled used
    #define <macro>(arg0, arg1) arg0 ## arg1
Macro is not ok to expand
...
* 第 12 行开始展开 `IDENTITY`，发现参数 `Token 0` 是 `CONCAT(...)`，也是一个宏，于是先对该参数宏进行展开。
...
{% highlight text linenos %}
HandleIdentifier: 
MacroInfo 0x55d9a9c988b0
    #define <macro>(arg0) arg0

HandleIdentifier: 
MacroInfo 0x55d9a9c989d0
    #define <macro>(arg0) IDENTITY_IMPL(arg0)
Macro is ok to expand

HandleIdentifier: 
MacroInfo 0x55d9a9c98780
    #define <macro>(arg0, arg1) arg0 ## arg1

EnterMacro

Enter ExpandFunctionArguments: 
MacroInfo 0x55d9a9c989d0 used
    #define <macro>(arg0) IDENTITY_IMPL(arg0)
Token: 0
...
HandleIdentifier: 
MacroInfo 0x55d9a9c98780 disabled used
    #define <macro>(arg0, arg1) arg0 ## arg1
Macro is not ok to expand
ResultArgToks: [identifier: CONCAT][l_paren: ][identifier: a][comma: ][identifier: b][r_paren: ]
Token: 3
...
* 第 16 行开始展开 `IDENTITY`，同理预处理器看到 `Token 2` （也即是 `arg0`）是宏，于是先展开 `CONCAT(C, ONCAT(a, b))`。

* 展开 `arg0` 后得到 `CONCAT(a, b)` （第 45-54 行）

* `IDENTITY` 最终展开为 `IDENTITY_IMPL(CONCAT(a, b))`（第 57 行）

* 重新扫描，继续展开 `IDENTITY_IMPL`（第 61 - 72 行），发现此时的 `Token 0` 是宏 `CONCAT(a, b)`，于是尝试展开，但发现是 `used` 状态，中止展开并返回（第 75-84行），最终得到的结果还是 `CONCAT(a, b)`（第 85 行）。

* 重新扫描结果，发现宏 `CONCAT(a, b)` 的状态是 `used`，停止展开并得到最终的结果。

通过以上三个简单的例子，我们可以大致的理解预处理器展开宏的过程，这里不再对预处理器进行更深入的探讨，有兴趣的可以对照我提供的修改文件来研究。

## 宏编程实现

下面我们开始进入到了主题（前面那一大段目的也是为了更好的理解宏展开规则），宏编程实现。
...
如果只是写 `PP_COMMA PP_LPAREN() PP_RPAREN()`，预处理器会当作单个内容来分别处理展开，对展开的结果不会再合并处理。加上 `PP_IDENTITY` 之后，预处理器可以对展开参数得到的 `PP_COMMA()` 再进行求值，得到 `,`。


### 

由于 `##` 拼接的时候，是不会对左右两边的参数进行展开了，为了让宏可以先求值再拼接，可以这样写：
...
借助 `PP_CNCAT` 可以实现逻辑运算。首先定义 `Bool` 值：
...
用`PP_CONCAT` 先把 `PP_BOOL_` 和 `arg0` 拼接在一起，再对拼接结果进行求值。这里的 `arg0` 要求是求值之后得到 `[0, 256]` 范围的数字，拼接在 `PP_BOOL_` 后面求值，就能得到布尔值。逻辑运算与或非：
...
PP_NOT(PP_BOOL(2))      // -> PP_CONCAT(PP_NOT_, PP_BOOL(2)) -> PP_NOT_1 -> 0
...
`if` 求值处理如果是 `1`，用 `PP_CONCAT` 拼接成 `PP_IF_1`，最后展开为 `then` 的值； `if` 求值为 `0` 也同理。
...
#define PP_IS_BEGIN_PARENS(...) \
    PP_IS_BEGIN_PARENS_PROCESS( \
        PP_IS_BEGIN_PARENS_CONCAT(PP_IS_BEGIN_PARENS_PRE_, PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__) \
    )
...
#define PP_IS_BEGIN_PARENS_PRE_PP_IS_BEGIN_PARENS_SWALLOW 0,
#define PP_IS_BEGIN_PARENS_SWALLOW(...) 1
...
`PP_IS_BEGIN_PARENS` 可以用来判断传入的参数是否以括号开始，在需要处理括号参数的时候会需要用到。

* `PP_IS_BEGIN_PARENS_PROCESS` 和 `PP_IS_BEGIN_PARENS_PROCESS_0` 组成的宏功能是先对传入的不定参数求值，然后取第 0 个参数。
* `PP_IS_BEGIN_PARENS_CONCAT(PP_IS_BEGIN_PARENS_PRE_, PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__)` 是先对 `PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__` 求值，在把求值结果跟 `PP_IS_BEGIN_PARENS_PRE_` 拼接在一起。
* `PP_IS_BEGIN_PARENS_SWALLOW(...)` 宏会吞掉所有参数，返回1，如果上一步 `PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__` 中，`__VA_ARGS__` 是以括号开始的，那么就会匹配到对 `PP_IS_BEGIN_PARENS_SWALLOW(...)` 的求值，然后返回 `1`；相反，如果不是以括号开始，则没有匹配上，`PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__` 会保留不变
* 若 `PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__` 求值得到 `1`，`PP_IS_BEGIN_PARENS_CONCAT(PP_IS_BEGIN_PARENS_PRE_, 1)` 得到 `1,`，注意 `1` 后面是有个逗号的，把 `1, ` 传给 `PP_IS_BEGIN_PARENS_PROCESS_0`，取第 0 个参数，最后得到 `1`，表示参数是以括号开始。
* 若 `PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__` 求值得到不是 `1`，而是保持不变，则 `PP_IS_BEGIN_PARENS_CONCAT(PP_IS_BEGIN_PARENS_PRE_, PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__) -> PP_IS_BEGIN_PARENS_PRE_PP_IS_BEGIN_PARENS_SWALLOW __VA_ARGS__ -> 0, __VA_ARGS__`，传给 `PP_IS_BEGIN_PARENS_PROCESS_0` 得到的是 `0`，表示参数不是以括号开始。

### 

判断变长参数是否为空也是一个常用的宏，在 C++ 20 之后引入了 `__VA_OPT__` 来表示，我们在这里也讨论一下实现的方式，利用 `PP_IS_BEGIN_PARENS`，可以先写出不完整的版本：

``` cpp
#define PP_IS_EMPTY_PROCESS(...) PP_IS_BEGIN_PARENS(PP_IS_EMPTY_PROCESS_NON_FUNC __VA_ARGS__ ())
#define PP_IS_EMPTY_PROCESS_NON_FUNC(...) ()
...
* `PP_IS_EMPTY_PROCESS` 的作用是判断 `PP_IS_EMPTY_PROCESS_NON_FUNC __VA_ARGS__ ()` 是否以括号开始。
* 如果 `__VA_ARGS__` 是空，`PP_IS_EMPTY_PROCESS_NON_FUNC __VA_ARGS__ () -> PP_IS_EMPTY_PROCESS_NON_FUNC() -> ()`，得到的是一对括号 `()`，再传给 `PP_IS_BEGIN_PARENS` 返回 `1`，表示参数是空。
* 否则，`PP_IS_EMPTY_PROCESS_NON_FUNC __VA_ARGS__ ()` 保持不变地传给 `PP_IS_BEGIN_PARENS`，返回 0，表示非空。

留意第 4 个例子 `PP_IS_EMPTY_PROCESS(()) -> 1`，`PP_IS_EMPTY_PROCESS` 不能正确处理以括号开始的变长参数，因为这样的变长参数会导致 `PP_IS_EMPTY_PROCESS_NON_FUNC` 被求值而得到 `()`。为了解决这个问题，我们需要区别对待参数是否以括号开始的情况：

``` cpp
#define PP_IS_EMPTY(...) \
    PP_IS_EMPTY_IF(PP_IS_BEGIN_PARENS(__VA_ARGS__))(PP_IS_EMPTY_ZERO, PP_IS_EMPTY_PROCESS)(__VA_ARGS__)
...
LOG("Hello %s\n", "World")      // -> printf("log: " "Hello %s\n", "World");
LOG("Hello World")              // -> printf("log: " "Hello World" ); 没有逗号，正常编译
```
...
PP_IF(ONE, PP_COMMA(), PP_LPAREN())     // -> PP_IF_1(,,)) -> 报错 unterminated argument list invoking macro "PP_IF_1"
```

我们知道，宏展开的时候会对参数进行求值，`PP_COMMA()` 和 `PP_LPAREN()` 求值之后再传给 `PP_IF_1`，得到 `PP_IF_1(,,))`，导致预处理出错。对于这种情况，可以采用一种叫做惰性求值方法：

``` cpp
PP_IF(ONE, PP_COMMA, PP_LPAREN)()       // -> PP_IF_1(PP_COMMA, PP_LPAREN)() -> PP_COMMA() -> ,
```
...
#define PP_ARGS_ELEM_8(a0, a1, a2, a3, a4, a5, a6, a7, a8, ...) a7

PP_ARGS_ELEM(0, "Hello", "World")   // -> PP_ARGS_ELEM_0("Hello", "World") -> "Hello"
PP_ARGS_ELEM(1, "Hello", "World")   // -> PP_ARGS_ELEM_1("Hello", "World") -> "World"
```

`PP_ARGS_ELEM` 的第一个参数是元素下标 `I`，后面的变长参数 `...` 是元素的列表。利用 `PP_CONCAT` 拼接 `PP_ARGS_ELEM_` 和 `I`，即可以得到相应取元素的宏 `PP_ARGS_ELEM_0..8`，再把变长参数传给该宏，展开返回下标对应位置的元素。
...
* 借用 `PP_ARGS_ELEM` 实现判断参数是否含有逗号 `PP_HAS_COMMA`。
* `PP_COMMA_ARGS` 会吞掉传入的任意参数，返回一个逗号。
* 判断变长参数是否为空的基础逻辑是 `PP_COMMA_ARGS __VA_ARGS__ ()` 返回一个逗号，也就是 `__VA_ARGS__` 为空，`PP_COMMA_ARGS` 和 `()` 拼接在一起变成对 `PP_COMMA_ARGS` 求值，具体的写法就是 `PP_HAS_COMMA(PP_COMMA_ARGS __VA_ARGS__ ())`。
* 但是会有例外的情况：
...
我们可以利用 `PP_IS_EMPTY` 来实现类似 `__VA_OPT__` 的宏：
...
* `PP_ARGS_OPT` 实现了类似 `__VA_OPT__` 的功能，当变长参数非空的时候，返回 `data` 参数，否则返回 `empty` 参数。
* 为了让 `data` 和 `empty` 支持逗号，要求两者都要用括号包住实际的参数，最后用 `PP_REMOVE_PARENS` 来移除外层的括号

有了 `PP_ARGS_OPT` 可以实现 `LOG2` 来模拟 `LOG` 实现的功能：

``` cpp
#define LOG2(format, ...) printf("log: " format PP_ARGS_OPT((,), (), __VA_ARGS__) __VA_ARGS__)

LOG2("Hello");                  // -> printf("log: " "Hello" );
LOG2("Hello %s", "World");      // -> printf("log: " "Hello %s" , "World");
```
